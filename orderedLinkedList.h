...
public:
    ...
    linkedListIterator<Type> lookUpBook(orderedLinkedList<bookType*> &, linkedListIterator<bookType*> & , string);
    ...
    else
        {
            ...
            if (current == nullptr) //Case 4
                ...
                //deleted is in the list
                {
                    ...
                    count--;
                    bookType::decBookCount();   //Update bookCount after deleting a book
                }
                else //Case 4
                    cout << "The item to be deleted is not in the " << "list." << endl;
...
//                                                                LOOK UP BOOK
//--------------------------------------------------------------------------------------------------------------------------------------------
//The module will be displayed below the inventory database module
//*****************************************************************
//*                  Enter your choice (1-5): 1
//							BOOK LOOK UP
// Search:  (user inputs a string to search for a book)
//
template <class Type>
linkedListIterator<Type>
	orderedLinkedList<Type>::lookUpBook(orderedLinkedList<bookType*> &other, linkedListIterator<bookType*> &it, string search)
{
	int findTil;
	string upTitle;
	bool found =  false;
	...
	for(find = it; find != other.end(); ++find)
	{

		upTitle = (*find)->getTitle();
		for(int i = 0; upTitle[i] != '\0'; i++)
			{
				if (upTitle[i] >= 'a' && upTitle[i] <= 'z')
					upTitle[i] = upTitle[i] - 32;        //transforming lowercase to uppercase
			}
		findTil = upTitle.find(search);
		if(findTil != string::npos)
			{
				found = true;
				break;
			}
